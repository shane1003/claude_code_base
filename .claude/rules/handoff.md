...
- 주의 지점: <동작이 바뀌는 곳, 마이그레이션·설정 변경, 되돌리기 어려운 것>

## 💬 커밋 추천
- **무엇을**: <이 커밋에 담기는 변경을 한 문장으로>
- **왜 지금**: <지금이 커밋 경계로 적절한 이유. 예: 기능 단위 완결 / 검증 통과 / 되돌릴 수 있는 최소 단위.
...
- **커밋 추천 · 왜 지금**: justify the commit *boundary*, not the change. A
  ...
  pass. If the tree is half-done, checks fail, or unrelated changes are
  mixed, say **보류 권고** with the reason and what would make it committable
  (e.g. "테스트 통과 후", "A와 B를 분리한 뒤"). Never recommend committing a
  broken state just because work stopped.
